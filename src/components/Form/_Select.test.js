...
describe("(Component) Select", () => {
  ...
  test("[componentWillReceiveProps] should update the state when the component receives new props", () => {
    ...
    comp.setProps({ options: [{ value: "NewF.js", text: "New Framework" }], sort: true })
  ...
  test("[handleKeyDown] should handle several keys", () => {
    const options = getOptions()
    const comp = mount(<Select options={options} selected={["jQuery", "Ember"]} sort/>)
    const inst = comp.instance()
  ...
  test("[open] should set the state", () => {
    const options = getOptions()
    const comp = mount(<Select options={options} selected={["jQuery", "Ember"]} disabled/>)
    const inst = comp.instance()
  ...
  test("[close] should set the state", () => {
    const options = getOptions()
    const comp = mount(<Select options={options} selected={["jQuery", "Ember"]}/>)
    const inst = comp.instance()
  ...
  test("[getSelectedOptions] should return the selected options or the placeholder if given", () => {
    ...
    expect(inst.getSelectedOptions()).toEqual([{ text: "my-placeholder", value: null }])
